"""This module provides tests for the User functionality."""
import vcr
...
def test_user_profile(profile_keys):
    ...
    response = User.user_instance.profile()
...
def test_user_following_list(follow_keys):
    ...
    response = User.user_instance.following_list(page=PAGE)
...
def test_user_follower_list(follow_keys):
    ...
    response = User.user_instance.follower_list(page=PAGE)
    response_test_list(response, follow_keys)
...
def test_user_tags(user_tags_keys):
    ...
    response = User.user_instance.user_tags(page=PAGE)
...
def test_user_pens(pen_keys):
    ...
    response = User.user_instance.pens(category=PENS_CATEGORY, tag=TAG, page=PAGE)
...
def test_user_posts(post_keys):
    ...
    response = User.user_instance.posts(category=POSTS_CATEGORY, page=PAGE)
...
def test_user_collections(collections_list_keys):
    ...
    response = User.user_instance.collections(category=COLLECTIONS_CATEGORY, page=PAGE)
